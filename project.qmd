...
Unemployment_by_month <- read_csv("UNRATE.csv")
before_after <- Unemployment_by_month |>
mutate( period = case_when( observation_date < as_date("2020-03-01") ~ "before",
  observation_date < as_date("2021-03-01") ~ "early",
  observation_date < as_date("2022-03-01") ~ "late",
  .default = "after" )) |>
  mutate( period = factor( period, levels = c("before","early","late","after") ) )

before_after |> 
  group_by( period ) |>
  summarize( minimum = quantile( UNRATE, probs = 0 ),
  Q1= quantile(UNRATE, probs = 0.25),
  median = quantile( UNRATE, probs = 0.5 ),
  Q3 = quantile (UNRATE, probs = 0.75), 
  maximum = quantile( UNRATE, probs = 1 )) |>
tt()
```
...
```{r, echo=FALSE, fig.width=24, fig.height=16}
#| include: FALSE
library(tidyverse)
library(sf)
library(tmap)
