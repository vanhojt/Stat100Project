...
tm_shape(tn)+
   tm_polygons()
tn <- tn |> 
  left_join(Unemployment_2015, by = c("NAME_2" = "County"))
```
...
unemployment <- read_csv("2020 unemplyment data - Sheet1.csv", show_col_types = FALSE) |>
  mutate( Unemployment_Rate = abs(Unemployment_Rate))
...
tm_shape(tn2)+
 ...
 tn2 <- tn2 |> 
  left_join(Unemplyment_2020, by = c("NAME_2" = "county")) 
```
